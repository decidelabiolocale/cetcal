...
$(document).ready(function () {
  ...
  $('#rav-homepage-envoyer').on('mousedown', function () {
    ...
    json_rav.rayon = parseFloat(($('#homepage-rav-rayon').val()));
  ...
  $('input').each(function () { $(this).attr('autocomplete', 'new-password'); })
});
...
$('producteur').each(function (i) {
  ...
  var typeprd = $(this).find('typeprd').text();
  ...
  */
  var typeprdtab = typeprd.split('µ')
  if (typeprdtab.length === 2) {
    switch (typeprdtab[0]) {
...
function ravElementFocus(pk_elem, dataType) {
  for (var i = 0; i < marker_collection.length; i++) {
    if (marker_collection[i].type.indexOf('Associations liées à la BIO Locale') === -1 && marker_collection[i].type.indexOf('AMAP') === -1 && marker_collection[i].type.indexOf('Magasin BIO') === -1 && marker_collection[i].type.indexOf('Marché') === -1 && dataType === 'producteur') {
      var html = marker_collection[i].mrkr._popup._content;
      var pk = $(html).attr('data');
      if (pk_elem == pk) {
        var latlng = marker_collection[i].mrkr._latlng;
        console.log('producteur '+ pk)
        console.log(marker_collection[i])
        console.log(latlng);
        cetcrtmainmap.flyTo([latlng.lat, latlng.lng], 14, { animate: false, duration: 0 });
        marker_collection[i].mrkr.fire('click');
      }
    } else if ((marker_collection[i].type.indexOf('Associations liées à la BIO Locale') !== -1 || marker_collection[i].type.indexOf('AMAP') !== -1 || marker_collection[i].type.indexOf('Magasin BIO') !== -1 || marker_collection[i].type.indexOf('Marché') !== -1) && dataType === 'entite') {
      var html = marker_collection[i].mrkr._popup._content;
      var pk = $(html).attr('data');
      if (pk_elem == pk) {
        var latlng = marker_collection[i].mrkr._latlng;
